...
async function executeTool(name: string, args: Record<string, string>): Promise<string> {
  try {
    ...
    if (name === "read_file")  return await readWorkspaceFile(args.path);
    if (name === "write_file") {
...
export async function runAgentStreaming(
  ...
  if (workspaceFiles.length) contextLines.push(`workspace_files:\n  ${workspaceFiles.join("\n  ")}`);
  if (selectedCode)          contextLines.push(`selected_code:\n\`\`\`\n${selectedCode}\n\`\`\``);
  ...
  while (true) {
    ...
    if (!message.tool_calls?.length) {
      ...
      if (selectedCode && (reply.includes("function") || reply.includes("const") || reply.includes("class"))) {
        ...
        if (match) await replaceSelectedCode(match[1].trim());
      }
      break;
    }
